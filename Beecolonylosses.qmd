...
-   The states with more colonies lost will also have more colonies added.
...
In this section we will be using the complete data set from the USDA where we only used half the data set in the exploratory analyses.

```{r}
ggplot(colony, aes(x = months, y = colony_lost, fill = months)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Colonies Lost Bar Graph", x = "Months", y = "Colonies Lost") +
  theme_minimal()
```
...
ggplot(colony, aes(x = months, y = colony_lost, fill = months)) +
  geom_violin() +
  labs(title = "Colonies Lost Violin Plot", x = "Months", y = "Colonies Lost") +
  theme_minimal()
```
